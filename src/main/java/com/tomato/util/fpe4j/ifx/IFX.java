...
/**
 * Implementation of the experimental IFX algorithm described in ifx-spec.pdf. The IFX algorithm is intended for
 * format-preserving encryption of strings of non-uniform symbols, e.g. mixed digits and letters.
 * <p>
 * To use this class, you must construct an instance, call initialize(int[]) with an array of radices to specify the
 * format of the strings to be encrypted, then call the encrypt() and decrypt() methods.
 * <p>
 * This algorithm was developed as a proof of concept for the encoding/decoding process, and the method of splitting
...
public class IFX {
    ...
    /**
     * Construct a new IFX instance and initialize the instance with a vector of radices.
     * <p>
     * Inputs: <br>
     ...
     * Y, a plaintext vector of length(W) integers such that 0 &lt;= Y[i] &lt; W[i] for all i in 0..length(W)
     * <p>
     * Outputs:<br>
     * X, a ciphertext vector of length(W) integers such that 0 &lt;= X[i] &lt; W[i] for all i in 0..length(W)
     * 
     * @param K
     ...
     * @param Y
     *            The ciphertext array of values with radices corresponding to W.
     ...
     * K, an AES key<br>
     * T, a vector of bytes of arbitrary length<br>
     * X, a plaintext vector of length(W) integers such that 0 &lt;= X[i] &lt; W[i] for all i in 0..length(W)
     * <p>
     * Outputs:<br>
     * Y, a ciphertext vector of length(W) integers such that 0 &lt;= Y[i] &lt; W[i] for all i in 0..length(W)
     * 
     * @param K
     *            The 128-, 192- or 256-bit AES key.
     * @param T
     ...
     * <p>
     * Inputs:<br>
     * X, a vector of length(W) integers such that 0 &lt;= X[i] &lt; W[i] for all i in 0..length(W)
     * <p>
     * Outputs:<br>
     * y, an unconstrained integer
     * 
     * @param X
     *            an array of integers of the same length as the array used to initialize this IFX instance
     ...
     * <p>
     * Output:<br>
     * Y, a vector of length(W) integers such that 0 &lt;= Y[i] &lt; W[i] for all i in 0..length(W)
     * 
     * @param y
     *            an integer value in the range 0..product(W)
     * @return an array of integers corresponding to the value of y and the radices W
     */
