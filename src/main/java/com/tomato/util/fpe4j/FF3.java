...
public class FF3 {
	...
	/**
	 * NIST SP 800-38G Algorithm 9: FF3.Encrypt(K, T, X) - Encrypt a plaintext string of numerals
	 ...
	 * @throws IllegalArgumentException
	 *             If T is not 8 bytes long; the length of X is not within the range
	 *             [minlen..maxlen]; or any value X[i] is not in the range [0..radix].
	 */
	public int[] encrypt(SecretKey K, byte[] T, int[] X, int n) throws InvalidKeyException {
		...
		if (T.length < 8) {
			throw new IllegalArgumentException("T must be an array of at least 8 bytes: " + T.length);
		}
		...
		if (n < minlen || n > maxlen) {
			throw new IllegalArgumentException("The length of X is not within the permitted range of " + minlen + ".." + maxlen + ": " + n);
		}
	...
	/**
	 * NIST SP 800-38G Algorithm 10: FF3.Decrypt(K, T, X) - Decrypt a ciphertext string of numerals
