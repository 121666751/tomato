...
import com.tomato.util.fpe4j.FFX.ArithmeticFunction;
import com.tomato.util.fpe4j.FFX.FFXParameters;
import com.tomato.util.fpe4j.FFX.FeistelMethod;
import com.tomato.util.fpe4j.FFX.RoundCounter;
import com.tomato.util.fpe4j.FFX.RoundFunction;
import com.tomato.util.fpe4j.FFX.SplitFunction;
...
public class FF3Parameters implements FFXParameters {
    ...
    /**
     * Arithmetic functions for the operations C = A [+] F K (n, T, i, B) in the encryption rounds and A = C [-] F K (n,
     * T, i, B) in the decryption rounds.
     * <p>
     * The FF3 algorithm requires a little more generality than the FFX algorithm allows, because it uses a modified
     * form of the arithmetic operations in the Feistel rounds.
     * <p>
     * FFX defines two types of arithmetic operators: blockwise arithmetic, which treats each input array as an integer
     * [0..radix<sup>m</sup>], where m is the length of the array, and which returns a result within the same range; and
     * characterwise arithmetic, which adds each array element individually and produces a resulting element within the
     * range [0..radix].
     * <p>
     * FF3 uses a modified form of blockwise arithmetic, where the sequence of elements in the first array is reversed
     * before the arithmetic operation, then the sequence of elements in the resulting array is reversed before it is
     * returned.
     */
