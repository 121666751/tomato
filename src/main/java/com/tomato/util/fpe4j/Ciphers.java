...
class Ciphers {
    ...
    /**
     * NIST SP 800-38G Algorithm 6: PRF(X) - Applies the pseudorandom function to the input using the supplied key.
     * <p>
     * Prerequisites:<br>
     ...
     * @return The output of the function PRF applied to the block X; PRF is defined in terms of a given designated
     *         cipher function.
     ...
     * <p>
     * The PRF(X) algorithm is an implementation of CBC mode encryption with a zero initialization vector. PRF(X) then
     * extracts the last block as the result. Instead of implementing CBC by hand, this method uses the Java libraries
     * to perform the same operation, and to demonstrate the equivalence of the methods.
     * 
     * @param K
     *            The AES key for the cipher function
     * @param X
    ...
    public byte[] prf2(SecretKey K, byte[] X) throws InvalidKeyException {
        ...
        try {
            byte[] Y = {(byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00, (byte)0x00,
     ...
     * <p>
     * Although the ECB mode of operation is not explicitly mentioned in NIST SP 800-38G, it is implied by the use of
     * the CIPH(X) function in FF1 and FF3.
     * <p>
     * To quote NIST SP 800-38G, "For both of the modes, the underlying block cipher shall be approved, and the block
     * size shall be 128 bits. Currently, the AES block cipher, with key lengths of 128, 192, or 256 bits, is the only
