...
/**
 * Implementation of the FFX algorithm described in
 ...
 * <p>
 * To use this class, you must construct an instance using either of the constructors, then call the encrypt() and/or
 * decrypt() methods. At a minimum, you must supply the split(n), rnds(n) and F(K,n,T,i,B) functions, along with the
 * other FFX parameters.
 * <p>
 * The round function F must be constructed from a block cipher or hash function. For AES, the CBC MAC and CMAC modes
 * are recommended. For hash functions, F could be constructed using HMAC.
 * <p>
 * At best, FFX is only as secure as the supplied parameter sets. At its worst, FFX may produce output that is less
 * secure than the underlying block cipher or hash function.
 * <p>
 * FFX itself, and this implementation in particular, may not be an efficient way to implement a specific FPE function
 * derived from FFX. The generality of FFX precludes some optimizations that may be possible in more direct
...
public class FFX {
    ...
    /**
     * The range of values for symbols in plaintexts and ciphertexts, 0..radix for each symbol.
     */
    ...
    /**
     * The arithmetic functions for the [+] and [-] operations in the Feistel rounds.
     */
    private final ArithmeticFunction arithmeticFunction;

    /**
     * The Feistel method, either ONE where the array is re-partitioned on each round, or TWO where the array partitions
     * are swapped on each round.
     */
    ...
    public interface SplitFunction {
        /**
         * The imbalance, a function that takes a permitted length n in the range [minlen..maxlen] and returns a number
         * 1 &lt;= split(n) &lt;= n/2.
         * 
         * @param n
         *            the length
    ...
    public interface RoundFunction {
         ...
         * 
         * @param K
         *            the encryption key of a type compatible with the underlying cipher
         * @param n
         ...
         * @return an array of the same length as A (i.e. n - B.length) with pseudorandom values in the range [0..radix]
         * @throws InvalidKeyException
         *             if the key is invalid or not compatible with the underlying cipher
         * @throws IllegalArgumentException
         *             If n is not within the range [minlen..maxlen]; the length of T is not within the range of
         *             [minTlen..maxTlen]; the length of B is not within the range [1..ceiling(n/2)]; ; or any value
         *             B[i] is not in the range [0..radix].
         */
    ...
    public interface ArithmeticFunction {
         ...
         * @return the result of X [+] Y with length = X.length = Y.length and elements each in the range [0..radix]
         * @throws NullPointerException
         *             if X or Y is null
         * @throws IllegalArgumentException
         *             if X.length != Y.length, or if any X[i] or Y[i] is not in the range [0..radix]
         */
    ...
    public interface FFXParameters {
        ...
        /**
         * @return the arithmetic functions for the [+] and [-] operations in the Feistel rounds
         */
    ...
    /**
     * Returns an ArithmeticFunction instance which implements blockwise arithmetic, treating each input array as an
     * integer [0..radix<sup>m</sup>], where m is the length of the array, and returning an array representing a result
     * within the same range.
     * 
     * @param radix
     *            the radix
     * @return an ArithmeticFunction which implements blockwise addition for the specified radix
     * 
     */
    public static ArithmeticFunction getBlockwiseArithmeticFunction(final int radix) {
        // validate radix
    ...
    /**
     * Returns an ArithmeticFunction instance which implements characterwise arithmetic, performing modulo arithmetic on
     * the corresponding elements of each input array, and returning an array of the same length as the input arrays
     * with elements in the range [0..radix].
     * 
     * @param radix
     *            the radix
     * @return an ArithmeticFunction which implements blockwise addition for the specified radix
     * 
     */
    public static ArithmeticFunction getCharwiseArithmeticFunction(final int radix) {
        // validate radix
        if (radix < Constants.MINRADIX) {
            throw new IllegalArgumentException("Radix must be greater than " + Constants.MINRADIX);
        }
     ...
     *            The maximum length of tweak inputs.
     * @param blockwise
     *            True if the functions for the [+] and [-] operations in the Feistel rounds are blockwise arithmetic,
     *            false if the functions use charwise arithmetic
     * @param method
     *            The Feistel method, either ONE where the array is re-partitioned on each round, or TWO where the array
    ...
    /**
     * FFX.Encrypt(K, T, X) - Encrypts a plaintext string of numerals and produces a ciphertext string of numerals of
     * the same length and radix.
     * <p>
     * 
     * @param K
     ...
     * @param X
     *            The plaintext numeral string with length in the range [minlen..maxlen] and element values in the range
     *            [0..radix].
     * @return The ciphertext numeral string of the same length with element values in the same range.
     * @throws NullPointerException
     *             If any of the arguments are null.
     * @throws IllegalArgumentException
     *             If the length of T is not within the range of [minTlen..maxTlen]; the length of X is not within the
     *             range [minlen..maxlen]; radix<sup>X.length</sup> is less than 100; or any value X[i] is not in the
     *             range [0..radix].
     * @throws InvalidKeyException
    ...
    /**
     * FFX.Decrypt(K, T, Y) - Decrypts a ciphertext string of numerals and produces a plaintext string of numerals of
     * the same length and radix.
     * <p>
     * 
     * @param K
     ...
     *            The tweak with length in the range [minTlen..maxTlen].
     * @param Y
     *            The ciphertext numeral string with length in the range [minlen..maxlen] and element values in the
     *            range [0..radix].
     * @return The plaintext numeral string of the same length with element values in the same range.
     * @throws NullPointerException
     *             If any of the arguments are null.
     * @throws IllegalArgumentException
     *             If the length of T is not within the range of [minTlen..maxTlen]; the length of X is not within the
