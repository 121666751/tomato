...
import static com.tomato.util.fpe4j.common.Common.*;
...
public class FF1 {
	 ...
	 *             If radix is not in the range [ {@value com.ifugle.util.fpe4j.Constants#MINRADIX}
	 *             .. {@value com.ifugle.util.fpe4j.Constants#MAXRADIX}]; or if maxTlen is not in
	 *             the range [0.. {@value com.ifugle.util.fpe4j.Constants#MAXLEN}].
	 */
	public FF1(int radix, int maxTlen) {
		// validate radix
		if (radix < Constants.MINRADIX || radix > Constants.MAXRADIX) {
			throw new IllegalArgumentException("Radix must be in the range [" + Constants.MINRADIX + ".." + Constants.MAXRADIX + "]: " + radix);
		}

		// validate maxTlen
		if (maxTlen < 0 || maxTlen > Constants.MAXLEN) {
			throw new IllegalArgumentException("maxTlen must be in the range [0.." + Constants.MAXLEN + "]: " + maxTlen);
		}
	...
	/**
	 * NIST SP 800-38G Algorithm 7: FF1.Encrypt(K, T, X) - Encrypt a plaintext string of numerals
	 * and produce a ciphertext string of numerals of the same length and radix.
	 * <p>
	 * Prerequisites:<br>
	 ...
	 * <p>
	 * Inputs:<br>
	 * Numeral string, X, in base radix of length n, such that n is in the range [minlen..maxlen];
	 * <br>
	 ...
	 * @throws IllegalArgumentException
	 *             If the length of T is not within the range of [0..maxTlen]; the length of X is
	 *             not within the range [ {@value com.ifugle.util.fpe4j.Constants#MINLEN}..
	 *             {@value com.ifugle.util.fpe4j.Constants#MAXLEN}]; radix <sup>X.length</sup> is
	 *             less than 100; or any value X[i] is not in the range [0..radix].
	...
	public int[] encrypt(SecretKey K, byte[] T, int[] X, int n) throws InvalidKeyException {
		...
		if (T.length > maxTlen) {
			throw new IllegalArgumentException("The length of T is not within the permitted range of 1.." + maxTlen + ": " + T.length);
		}
		...
		if (n < Constants.MINLEN || n > Constants.MAXLEN) {
			throw new IllegalArgumentException("The length of X is not within the permitted range of " + Constants.MINLEN + ".." + Constants.MAXLEN + ": " + n);
		}
		if (Math.pow(radix, n) < 100) {
			throw new IllegalArgumentException("The length of X must be such that radix ^ length > 100 (radix ^ length =" + Math.pow(radix, n));
		}

		if (Constants.CONFORMANCE_OUTPUT) {
			System.out.println("FF1.Encrypt()\n");
			System.out.println("X is " + intArrayToString(X, n));
		...
		byte[] P = { (byte) 0x01, (byte) 0x02, (byte) 0x01, tbr[0], tbr[1], tbr[2], (byte) 0x0A, (byte) (mod(u, 256) & 0xFF), fbn[0], fbn[1], fbn[2], fbn[3],
	...
	/**
	 * NIST SP 800-38G Algorithm 8: FF1.Decrypt(K, T, X) - Decrypt a ciphertext string of numerals
	 * and produce a plaintext string of numerals of the same length and radix.
	 * <p>
	 * Prerequisites: <br>
	 * Designated cipher function, CIPH, of an approved 128-bit block cipher;<br>
	 * Key, K, for the block cipher;<br>
