...
import static com.tomato.util.fpe4j.Common.byteArrayToHexString;
import static com.tomato.util.fpe4j.Common.bytestring;
import static com.tomato.util.fpe4j.Common.ceiling;
import static com.tomato.util.fpe4j.Common.concatenate;
import static com.tomato.util.fpe4j.Common.floor;
import static com.tomato.util.fpe4j.Common.log2;
import static com.tomato.util.fpe4j.Common.mod;
import static com.tomato.util.fpe4j.Common.num;
import static com.tomato.util.fpe4j.Common.str;
import static com.tomato.util.fpe4j.Common.xor;
...
/**
 * Implementation of the FF1 method for format-preserving encryption defined in NIST SP 800-38G.
 * <p>
 * To use this class, construct an instance and call the encrypt() and/or decrypt() methods.
 * 
 * @author Kai Johnson
 *
 */
public class FF1 {
     ...
     *             If radix is not in the range [{@value com.tomato.util.fpe4j.Constants#MINRADIX}..
     *             {@value com.tomato.util.fpe4j.Constants#MAXRADIX}]; or if maxTlen is not in the range [0..
     *             {@value com.tomato.util.fpe4j.Constants#MAXLEN}].
     */
    ...
    /**
     * NIST SP 800-38G Algorithm 8: FF1.Decrypt(K, T, X) - Decrypt a ciphertext string of numerals and produce a
     * plaintext string of numerals of the same length and radix.
     * <p>
     * Prerequisites: <br>
     * Designated cipher function, CIPH, of an approved 128-bit block cipher;<br>
     ...
     * <p>
     * Inputs:<br>
     * Numeral string, X, in base radix of length n, such that n is in the range [minlen..maxlen];<br>
     * Tweak T, a byte string of byte length t, such that t is in the range [0..maxTlen].
     * <p>
     * Output:<br>
     ...
     * @param n
     *            number of readability (added by @author WuJianqiang)
     * @return The plaintext numeral string of the same length and radix.
     * @throws NullPointerException
     *             If any of the arguments are null.
     * @throws IllegalArgumentException
     *             If the length of T is not within the range of [0..maxTlen]; the length of X is not within the range [
     *             {@value com.tomato.util.fpe4j.Constants#MINLEN}..{@value com.tomato.util.fpe4j.Constants#MAXLEN}];
     *             radix<sup>X.length</sup> is less than 100; or any value X[i] is not in the range [0..radix].
     * @throws InvalidKeyException
     *             If K is not a valid AES key.
     */
    public int[] decrypt(SecretKey K, byte[] T, int[] X, int n) throws InvalidKeyException {
        // validate K
        if (K == null) {
        ...
        if (n < Constants.MINLEN || n > Constants.MAXLEN) {
            throw new IllegalArgumentException("The length of X is not within the permitted range of "
                + Constants.MINLEN + ".." + Constants.MAXLEN + ": " + n);
        }
        ...
        if (Constants.CONFORMANCE_OUTPUT) {
            ...
            System.out.println("X is " + Common.intArrayToString(X, n));
        ...
        byte[] P = {(byte)0x01, (byte)0x02, (byte)0x01, tbr[0], tbr[1], tbr[2], (byte)0x0A, (byte)(mod(u, 256) & 0xFF),
            fbn[0], fbn[1], fbn[2], fbn[3], fbt[0], fbt[1], fbt[2], fbt[3]};
        if (Constants.CONFORMANCE_OUTPUT) {
    ...
    public int[] decrypt(SecretKey K, byte[] T, int[] X) throws InvalidKeyException {
        return decrypt(K, T, X, 0);
    }

    /**
     * NIST SP 800-38G Algorithm 7: FF1.Encrypt(K, T, X) - Encrypt a plaintext string of numerals and produce a
     * ciphertext string of numerals of the same length and radix.
     * <p>
     * Prerequisites:<br>
     * Designated cipher function, CIPH, of an approved 128-bit block cipher;<br>
     * Key, K, for the block cipher; <br>
     ...
     * Inputs:<br>
     * Numeral string, X, in base radix of length n, such that n is in the range [minlen..maxlen];<br>
     * Tweak T, a byte string of byte length t, such that t is in the range [0..maxTlen].<br>
     * <p>
     * Output:<br>
     * Numeral string, Y, such that LEN(Y) = n.
     * 
     ...
     *             If K is not a valid AES key.
     */
    public int[] encrypt(SecretKey K, byte[] T, int[] X, int n) throws InvalidKeyException {
        // validate K
        if (K == null) {
            throw new NullPointerException("K must not be null");
        }
        ...
        if (Math.pow(radix, n) < 100) {
            throw new IllegalArgumentException(
                "The length of X must be such that radix ^ length > 100 (radix ^ length =" + Math.pow(radix, n));
        }
