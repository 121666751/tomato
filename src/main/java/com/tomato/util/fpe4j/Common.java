...
class Common {
     ...
     */
    private static final String HEX_STRINGS[] = {"00", "01", "02", "03", "04", "05", "06", "07", "08", "09", "0A", "0B",
     ...
     * <p>
     * Note that although NIST SP 800-38G refers to strings of individually addressable bits, this is not a common
     * feature of programming languages. In this implementation, we use arrays of bytes to represent the data type.
     * <p>
     * The implementations of FF1 and FF3 sometimes use inline constants in place of this bitstring function.
     * <p>
     * NIST SP 800-38G notation: 0<sup>s</sup> or 1<sup>s</sup>
     * 
     * @param bit
     *            The bit value with which to fill the output; false = 0, true = 1
     * @param s
    ...
    /**
     * Converts an array of bytes to a string representing the hexadecimal value of the bytes.
     * 
     * @param X
     *            The array of bytes.
     ...
     * @return Given a nonnegative integer x less than 256<sup>s</sup> , the representation of x as a string of s bytes.
     * @throws IllegalArgumentException
     *             If s is not within the range [1..{@value com.tomato.util.fpe4j.Constants#MAXLEN}]; if x is negative;
     ...
     * @return Given a nonnegative integer x less than 256<sup>s</sup> , the representation of x as a string of s bytes.
     * @throws IllegalArgumentException
     *             If s is not within the range [0..{@value com.tomato.util.fpe4j.Constants#MAXLEN}]; if x is negative;
     *             or if x is greater than 256<sup>s</sup>.
     */
     ...
     * <p>
     * In NIST SP 800-38G, this function is used to round the result of integer division. However, in Java, the result
     * of integer division is rounded down to an integer. To use the function on the result of integer division, one of
     * the integers must first be converted to a double to produce a fractional result.
     * <p>
    ...
    /**
     * Alternate form of the ceiling function with an integer argument to trap incorrect usage.
     * 
     * @param x
     *            An integer.
    ...
    /**
     * Alternate form of the floor function with an integer argument to trap incorrect usage.
     * 
     * @param x
     *            An integer.
     * @return Throws an IllegalArgumentException.
    ...
    /**
     * Converts an array of integers to a string, with spaces separating the integer values.
     * 
     * @param X
     *            The array of integers.
     * @param len
     *            length of used. (added by @author WuJianqiang)
     * @return The string with spaces separating the elements of X.
     * @throws NullPointerException
     *             If X is null.
     */
    static String intArrayToString(int[] X, int len) {
        // validate X
        if (X == null) {
            throw new NullPointerException("X must not be null");
        }
     ...
     * <p>
     * We have renamed this function "log2(x)" to avoid confusion with the natural log function.
     * <p>
     * NIST SP 800-38G describes the input as a "real" number, but only provides integer values as input, so we have
     * defined the input as an int to avoid unnecessary type conversion.
     * <p>
    ...
    /**
     * Given a real number x and a positive integer m, returns the remainder of x modulo m, denoted by x mod m, which is
     * x - m * floor(x/m).
     * <p>
     * NIST SP 800-38G describes the input as a "real" number, but provides only integer values as input. We have
     * defined the input as a BigInteger to avoid unnecessary type conversion.
     * <p>
     * NIST SP 800-38G Notation: MOD(x)
     * 
     * @param x
     *            The "real" number (defined as a BigInteger to avoid unnecessary type conversion).
     * @param m
     *            The modulus.
     * @return The nonnegative remainder of the integer x modulo the positive integer m.
     ...
     * 
     * @param x
     *            The "real" number (defined as an int to avoid unnecessary type conversion).
     * @param m
     *            The modulus.
     * @return The nonnegative remainder of the integer x modulo the positive integer m.
     * @throws ArithmeticException
    ...
    /**
     * NIST SP 800-38G Algorithm 2: NUM(X) - Converts a string of bytes to an integer, valuing the bytes as unsigned
     * integers in decreasing order of significance.
     * <p>
     * Input:<br>
     ...
     * Integer, x.
     * <p>
     * Note that NIST SP 800-38G defines the algorithm in terms of bits, but we have implemented it using an array of
     * bytes as input.
     * 
     * @param X
     *            The string of bytes to convert to a number.
     * @return The integer that a byte string X represents when the bytes are valued in decreasing order of
     *         significance.
     * @throws NullPointerException
     *             If X is null.
     * @throws IllegalArgumentException
     *             If X is empty or if X has more than {@value com.tomato.util.fpe4j.Constants#MAXLEN} elements.
     */
    ...
    /**
     * NIST SP 800-38G Algorithm 1: NUM<sub>radix</sub>(X) - Converts a string of numerals to an integer, valuing the
     * numerals in decreasing order of significance.
     * <p>
     * Prerequisite:<br>
     * Base, radix.
     * <p>
     ...
     * @param radix
     *            The base of the numerals such that 0 &lt;= X[i] &lt; radix for all i.
     * @return The number that the numeral string X represents in base <i>radix</i> when the numerals are valued in
     *         decreasing order of significance.
     * @throws NullPointerException
     *             If X is null.
     * @throws IllegalArgumentException
     *             If X is empty or has more than {@value com.tomato.util.fpe4j.Constants#MAXLEN} elements; if radix is
     *             less than {@value com.tomato.util.fpe4j.Constants#MINRADIX} or greater than
     *             {@value com.tomato.util.fpe4j.Constants#MAXRADIX}; or if any numeral X[i] is outside the range
     *             [0..radix-1].
     */
     ...
     * @return Given a numeral string, X, the numeral string that consists of the numerals of X in reverse order.
     * @throws NullPointerException
     *             If X is null.
     */
     ...
     * @return Given a byte string, X, the byte string that consists of the bytes of X in reverse order.
     * @throws NullPointerException
     *             If X is null.
     */
    ...
    /**
     * NIST SP 800-38G Algorithm 3: STR<sup>m</sup><sub>radix</sub>(x) - Converts an integer to an array of numerals of
     * a given radix.
     * <p>
     * Prerequisites:<br>
     * Base, radix;<br>
     ...
     * @return Given a nonnegative integer x less than radix<sup>m</sup>, the representation of x as a string of m
     *         numerals in base radix, in decreasing order of significance.
     * @throws NullPointerException
     *             If x is null.
     * @throws IllegalArgumentException
     *             If m is not within the range [1..{@value com.tomato.util.fpe4j.Constants#MAXLEN}]; if radix is not
     *             within the range [{@value com.tomato.util.fpe4j.Constants#MINRADIX}..
     *             {@value com.tomato.util.fpe4j.Constants#MAXRADIX}]; or if x is not within the range [0..radix
     *             <sup>m</sup>].
     */
    ...
    /**
     * Converts an array of bytes to a string, treating the bytes as unsigned integers, with commas and spaces
     * separating the byte values.
     * 
     * @param X
     *            The array of bytes.
     * @return The string with commas and spaces separating the elements of X interpreted as unsigned values.
     * @throws NullPointerException
     *             If X is null.
     */
     ...
     * @return The bitwise addition, modulo 2, of two bit strings of equal length.
     * @throws NullPointerException
     *             If either X or Y is null.
     * @throws IllegalArgumentException
     *             if X or Y is empty; if X or Y has more than {@value com.tomato.util.fpe4j.Constants#MAXLEN} elements;
