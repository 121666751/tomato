...
public class Path {
	 ...
	 * @param revSeparator the rev separator
	 */
	...
	/**
	 * Is allow duplicated separator boolean.
	 *
	...
	/**
	 * Sets allow duplicated separator.
	 *
	 * @param allowDuplicatedSeparator the allowDuplicatedSeparator to set
	 */
	 ...
	 *
	 * @param s 要追加的序列。
	 * @param start 要追加的子序列的起始索引。允许负数为倒数计数，此时增加 s.length() 自动调整为正向索引。如果 start 仍然为负，则自动调整为 0
	 * @param end 要追加的子序列的结束索引。如果 end 为负或者 end 大于 s.length() 则自动调整为 s.length()。通常可用 -1 表示序列尾部，这样不必检查            s 是否为 null
	 *
	 * @return 此对象的一个引用 。
	 */
